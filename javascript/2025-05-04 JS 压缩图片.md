...
在实际应用中有可能使用的情境：大多时候我们直接读取用户上传的 `File` 对象，读写到画布（`canvas`）上，利用 `Canvas` 的 `API` 进行压缩，完成压缩之后再转成 `File（Blob）` 对象，上传到远程图片服务器；

不妨有时候我们也需要将一个 `base64` 字符串压缩之后再变为 `base64` 字符串传入到远程数据库或者再转成 `File（Blob）` 对象。

一般的，它们有如下转化关系：

![js-image-compressor-flow-chat](https://segmentfault.com/img/bVbKHRd)
...
用户通过页面标签 `<input type="file" />` 上传的本地图片直接转化 [`data URL`](https://link.segmentfault.com/?enc=TNN48CbBfRfntRnfLFnvWA%3D%3D.4ha6%2FhYDnk1tDYYaw1lmoW5bdeZ%2Bsk4kq8kckLWqmqy53qYBkY3SYJJRQCjXvDf7HD2uq9KrW7Bj6UUBrTgbfw%3D%3D) 字符串形式。

可以使用 [`FileReader`](https://link.segmentfault.com/?enc=bXFIeX29x2PCACiS3GSM0w%3D%3D.S1bH%2FWobZaJqFKe%2BIAWGMFh2gdja1CwZWJUbRwWRR4YzMBjNsugq71qLVHRuumiPOuA3xV%2BXVbMmgr2qpa2HKw%3D%3D) 文件读取构造函数。

`FileReader` 对象允许 `Web` 应用程序异步读取存储在计算机上的文件（或原始数据缓冲区）的内容，使用 [`File`](https://link.segmentfault.com/?enc=9a3k20NzVdtiKXC4IabfOg%3D%3D.GVtwaArZ9Q%2B%2F7NrgMxQ6s0xLOeA7udqEhx0u3je%2BD9FR7J8rdPr6xMx8TteEp3VDMPK4ud%2BRZzgV9VmFAG%2BLqA%3D%3D) 或 [`Blob`](https://link.segmentfault.com/?enc=%2FqkFgoeEegtPvSF7%2FFVP%2Fw%3D%3D.YjwGhAr2682SF3rw3A3A6%2B1OndiQt%2B1EMTL3sy9iSaoNIkoktN%2F%2BCYMaxLJfshqUXx3S8L7baGNSN%2BdhVJ%2FDfA%3D%3D) 对象指定要读取的文件或数据。
...
注意：要创建对象 `URL`，可以使用 `window.URL.createObjectURL()` 方法，并传入 `File` 或 `Blob` 对象。

如果不再需要相应数据，最好释放它占用的内容。但只要有代码在引用对象 `URL`，内存就不会释放。要手工释放内存，可以把对象 `URL` 传给 `URL.revokeObjectURL()`。
...
利用 [`drawImage()`](https://link.segmentfault.com/?enc=%2B6SjQf796m0OEQpd%2BP8hQQ%3D%3D.P0CQDXK4jdFrG2eM9ddsa8OIeIywddpzkiUlfLORE6DFpQctySaOx3Ujuag2N4BXGwht7%2FMWJQgY78ndxWjgU2HragOIgmkWyz59R5JX9SJ08c0Fr53eP1vqP0YCee2m) 方法将 `Image` 对象绘画在 `Canvas` 对象上。
...
![canvas-draw-image](https://segmentfault.com/img/bVdkst)
...
`HTMLCanvasElement` 对象有 `toDataURL(type, encoderOptions)` 方法，返回一个包含图片展示的 [`data URL`](https://link.segmentfault.com/?enc=cCgWADfQsAga1SZjCUZmag%3D%3D.MQadz2nvuNuRRd0Lyx%2F9Qz5Xv7tyIc5KAn%2BeCbGS47TvBwnTFqELGSZpp6wpEtfnAqEnmUkG3BjQlRrDWkeQAg%3D%3D) 。同时可以指定输出格式和质量。
...
将 `data URL` 字符串转化为 [`Blob`](https://link.segmentfault.com/?enc=mEI%2FwijmF9mcIAQd0NPy%2FQ%3D%3D.C5HIm1YW%2BKipPBUPFScbMSBF9VVgMko1gOvZZmJ3cU9mRqAuXnJ4U%2BPgkKHHFD0%2FxSJqWvdVgzDWFEjTH9Z67Q%3D%3D) 对象。主要思路是：先将 `data URL` 数据（`data`） 部分提取出来，用 `atob` 对经过 `base64` 编码的字符串进行解码，再转化成 `Unicode` 编码，存储在`Uint8Array`（8位无符号整型数组，每个元素是一个字节） 类型数组，最终转化成 `Blob` 对象。
...
`HTMLCanvasElement` 有 [`toBlob(callback, [type\], [encoderOptions])`](https://link.segmentfault.com/?enc=WW2Vui826i8NnfFLnmWNVQ%3D%3D.uuNEzPFmuv6VnFEc7tHy7FewMF5d%2FTRoYJ5CFwV9VXL2xlnco7Jvyv1mCfwcRT%2F8yuxDfltQ0nAkNjH8o5KdK17kVaYKZwQOzd3tHPJsFD0%3D) 方法创造 `Blob` 对象，用以展示 `canvas` 上的图片；这个图片文件可以被缓存或保存到本地，由用户代理端自行决定。第二个参数指定图片格式，如不特别指明，图片的类型默认为 `image/png`，分辨率为 `96dpi`。第三个参数用于针对`image/jpeg` 格式的图片进行输出图片的质量设置。
...
如果看到这里的客官不嫌弃这个 `demo` 太简单可以戳[这里](https://link.segmentfault.com/?enc=Mju4yt4bMqqgoiVhLXdrBA%3D%3D.9Nuiw%2BAEU1ymX%2Bxlxn%2FiYQo0FDhQqAmtFYxfhe1RkT6N0QNf8x43HcJW72KDTUHTyHijLOTIyiS7Z%2Bo3sYg6Tg%3D%3D)试试水。如果你有足够的耐心多传几种类型图片就会发现还存在如下问题：
...
![越压缩越膨胀😂](https://segmentfault.com/img/bVbKHRx)
...
由于各大浏览器对 [`Canvas` 最大尺寸支持](https://link.segmentfault.com/?enc=67EK6%2F%2BRLx8A232PC8RGew%3D%3D.VX4ee7NJJ%2FyJ4sWlxKHeQedkKoXsW2d55E8IaKY%2BCvjB%2BdXS4OrW0m2qupC5dVQyaXPcxv8fPABDqXud%2B7iCim2%2BEYKj59WWbm6GdrQ950Y%3D)不同
...
到这里，上述的意外问题被我们一一解决了，如需体验改进版的图片压缩 `demo` 的小伙伴可以戳[这里](https://link.segmentfault.com/?enc=I%2BPNV7RmbqJAO6UEC7%2FYjA%3D%3D.CM31wLY9Paoo4biLVF3fmmFikxwopbBiKnl1YmEBnTtPqrVB7MZVI5PHiziBzFqWww2xRpYyeAPhLieKga3qPg%3D%3D)
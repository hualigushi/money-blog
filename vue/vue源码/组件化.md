...
_update 过程中有几个关键的代码，首先 `vm._vnode = vnode` 的逻辑，这个 vnode 是通过 `vm._render()` 返回的组件渲染 VNode，
...
如果组件的根节点是个普通元素，那么 vm._vnode 也是普通的 vnode，这里 `createComponent(vnode, insertedVnodeQueue, parentElm, refElm)` 的返回值是 false。

![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5df21d55fa414165b9e12784334ab79e~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)

上一节课我们讲解了 `step3`、`step4` 的实现原理，接下来的 `6`、`7`、`8` 三节课程都会讲述 `step5` 的实现原理。分成 3 个小节是因为 `step5` 做的事情可以很清晰地划分为三个小步骤，即针对每一个需要分析的 TS 文件：

![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/78bd99a4b2044c448cd849faf65dbb9d~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)

1. 遍历其所有 import 节点（上图绿框区域），分析并记录从目标依赖中导入的 `API` 信息，排除非目标依赖项的干扰。
2. 判定导入的 `API` 在具体代码中（上图红框区域）是否有调用，过程中还需要排除局部同名变量等一系列干扰。
...
> 本节涉及的完整源码在 [lib/analysis.js](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fliangxin199045%2Fcode-analysis-ts%2Fblob%2Fmain%2Flib%2Fanalysis.js) 中的 116 - 207 行，即 _findImportItems 方法的具体逻辑。
...
我们结合 [TypeScript AST Viewer](https://link.juejin.cn/?target=https%3A%2F%2Fts-ast-viewer.com%2F%23code%2FATCWFsAcHsCcBdgG9gFMB2A3UtrvBogL7ABmu4wA5OQIYEDucA1lQNwiecD03w69VABMwUOPABQICDATJgsVAEcArqgDOiWuoXLgJctEo1YgprFYcuvfoPWRaAY2GjZk6WLm1IoMhWp0jCzsXKFhoTZCqKS0KgA2iDLiUq7iwABUwNrAAIIACnl%2BRgGmQRYh4eE2AgT2Ti4Q4kA) 观察一下上面几种 Import 语句的 AST 结构：

![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/dd42bc99333e43d187528ec77559f27e~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e42cb5c3707a47dbb05ef63d46143db6~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)

[TypeScript AST Viewer](https://link.juejin.cn/?target=https%3A%2F%2Fts-ast-viewer.com%2F%23code%2FATCWFsAcHsCcBdgG9gFMB2A3UtrvBogL7ABmu4wA5OQIYEDucA1lQNwiecD03w69VABMwUOPABQICDATJgsVAEcArqgDOiWuoXLgJctEo1YgprFYcuvfoPWRaAY2GjZk6WLm1IoMhWp0jCzsXKFhoTZCqKS0KgA2iDLiUq7iwABUwNrAAIIACnl%2BRgGmQRYh4eE2AgT2Ti4Q4kA) 中的 SourceFile 表示 AST Root 节点，把它命名为 ast，获取节点的相关信息：
...
这里大家需要注意一点，AST 对象节点的`属性名`与其`类型名`在命名上并非一致，我们在节点树状图中展示的是各个节点的类型名而非属性名。例如，`ast.statements[0].importClause.namedBindings` 的节点类型是 `NamedImports`。可以通过 [TypeScript AST Viewer](https://link.juejin.cn/?target=https%3A%2F%2Fts-ast-viewer.com%2F%23code%2FATCWFsAcHsCcBdgG9gFMB2A3UtrvBogL7ABmu4wA5OQIYEDucA1lQNwiecD03w69VABMwUOPABQICDATJgsVAEcArqgDOiWuoXLgJctEo1YgprFYcuvfoPWRaAY2GjZk6WLm1IoMhWp0jCzsXKFhoTZCqKS0KgA2iDLiUq7iwABUwNrAAIIACnl%2BRgGmQRYh4eE2AgT2Ti4Q4kA) 或者 [AST explorer](https://link.juejin.cn/?target=https%3A%2F%2Fastexplorer.net%2F%3Fspm%3Dtaofed.bloginfo.blog.3.3ba15ac8enL4hJ) 工具来对应 `属性名` 与 `类型名` 。
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/de484b37004449179dfef4f676130f2c~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/8ae9c5a689514c47a7273ae3ad507cc8~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b076b2121b54404985645fc2f6cc3d06~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
2. `importClause` 对象如果只有 `name` 属性，没有 `namedBindings` 属性，那么可以判定为默认全局导入；
3. `importClause` 对象存在 `namedBindings` 属性，且类型为 `NamespaceImport`，则可以判定为全局别名导入；
4. `importClause` 对象存在 `namedBindings` 属性，并且类型为 `NamedImports`，并且 `elements` 属性为数组，并且长度大于 0。遍历 elements 数组的每一个元素，如果该元素的类型为 ImportSpecifier，则可以判定其属于局部导入。至于它是否存在 as 别名，则需要进一步判断其是否存在 propertyName 属性与 name 属性。如果都存在，则说明其属于局部别名导入。如果只有 name 属性，就为常规局部导入。`elements` 对象是一个数组，对于多个局部导入的场景上述规则也适用。
...
上述代码中提取 ast 相关属性的逻辑建议大家结合 [TypeScript AST Viewer](https://link.juejin.cn/?target=https%3A%2F%2Fts-ast-viewer.com%2F%23code%2FATCWFsAcHsCcBdgG9gFMB2A3UtrvBogL7ABmu4wA5OQIYEDucA1lQNwiecD03w69VABMwUOPABQICDATJgsVAEcArqgDOiWuoXLgJctEo1YgprFYcuvfoPWRaAY2GjZk6WLm1IoMhWp0jCzsXKFhoTZCqKS0KgA2iDLiUq7iwABUwNrAAIIACnl%2BRgGmQRYh4eE2AgT2Ti4Q4kA) 一起理解，我们用 `importItems` 这个 Map 结构来收集导入的 API 信息，收集的逻辑抽离在 `dealImports` 子函数中。
...
2. 不同 import 导入方式在 `API` 表述上存在差异，分析程序需要区分它们，不然无法获取准确的 `API` 导入信息。
3. Import 节点分析不光要记录导入到 `API` 调用名，还需要记录 `Symbol`、`as` 映射，节点字符流位置等信息。

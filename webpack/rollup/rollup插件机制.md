...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3bd4a66beffc4b6182498a4bda07ce8f~tplv-k3u1fbpfcp-watermark.awebp)
...
官方插件维护在 [rollup/plugins](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Fplugins) 仓库，社区精选插件维护在 [rollup/awesome](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Fawesome)。如果你想给某个插件提建议，请提交一个 pr。
...
构建钩子函数在构建阶段执行，它们被 [`rollup.rollup(inputOptions)`](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2F07b3a02069594147665daa95d3fa3e041a82b2d0%2Fcli%2Frun%2Fbuild.ts%23L34) 触发。

它们主要关注在 Rollup 处理输入文件之前定位、提供和转换输入文件。

构建阶段的第一个钩子是 `options`，最后一个钩子总是 `buildEnd`，除非有一个构建错误，在这种情况下 `closeBundle` 将在这之后被调用。
...
它们和构建钩子函数拥有一样的工作原理和相同的类型，但是不同的是它们分别被 ·[`bundle.generate(output)`](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2F07b3a02069594147665daa95d3fa3e041a82b2d0%2Fcli%2Frun%2Fbuild.ts%23L44) 或 [`bundle.write(outputOptions)`](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2F07b3a02069594147665daa95d3fa3e041a82b2d0%2Fcli%2Frun%2Fbuild.ts%23L64) 调用。

只使用输出生成钩子的插件也可以通过输出选项传入，因为只对某些输出运行。

输出生成阶段的第一个钩子函数是 [outputOptions](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2F07b3a02069594147665daa95d3fa3e041a82b2d0%2Fsrc%2FBundle.ts%23L50)，如果输出通过 [bundle.generate(...)](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2Fmaster%2Fcli%2Frun%2Fbuild.ts%23L44) 成功生成则第一个钩子函数是 [generateBundle](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2Fmaster%2Fsrc%2FBundle.ts%23L73)，如果输出通过 [`bundle.write(...)`](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2F07b3a02069594147665daa95d3fa3e041a82b2d0%2Fsrc%2Fwatch%2Fwatch.ts%23L200) 生成则最后一个钩子函数是 [`writeBundle`](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2Fmaster%2Fsrc%2Frollup%2Frollup.ts%23L176)，另外如果输出生成阶段发生了错误的话，最后一个钩子函数则是 [renderError](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2Fmaster%2Fsrc%2FBundle.ts%23L70)。

另外，[closeBundle](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2Fmaster%2Fsrc%2Frollup%2Frollup.ts%23L59) 可以作为最后一个钩子被调用，但用户有责任手动调用 `bundle.close()` 来触发它。CLI 将始终确保这种情况发生。

#### 钩子函数加载实现

[`PluginDriver`](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Frollup%2Frollup%2Fblob%2F07b3a02069594147665daa95d3fa3e041a82b2d0%2Fsrc%2Futils%2FPluginDriver.ts%23L124) 中有 9 个 hook 加载函数。主要是因为每种类别的 hook 都有同步和异步的版本。

![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2bba7776f98d4cbc9d184101d1f1b752~tplv-k3u1fbpfcp-watermark.awebp)
...
- [yargs-parser](https://link.juejin.cn?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Fyargs-parser)：yargs 使用的强大的选项解析插件
- [source-map-support](https://link.juejin.cn?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Fsource-map-support)：这个模块通过 V8 堆栈追踪 API 支持 堆栈 sourcemap 支持

## 总结

Rollup 的插件和其他大型框架大同小异，都是提供统一的接口并贯彻了约定优于配置的思想。

9 种 hook 加载函数使 rollup 的插件开发非常灵活，同时也带来了学习成本。

和 webpack 相比，rollup 的插件系统自称一派且没有区分 plugin 和 loader。

Rollup 插件机制的核心是构建阶段和输出生成阶段的各种钩子函数。内部通过基于 Promise 实现异步 hook 的调度。

rollup 的源码全都糅杂在一个库中，阅读起来着实头大，模块、工具函数管理的看起来很随意。而且我们无法直接移植它的任何工具到我们的项目中，相比起来，webpack 的插件系统封装成了一个插件 [tapable](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Fwebpack%2Ftapable) 就很利于我们学习和使用。